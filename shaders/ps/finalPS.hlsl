// Final screen pass: bloom add, exposure, filmic tone map, grading matrix
// (YCbCr tweak or identity) and optional PS2-style 15-bit dithering.
...
{
	float3 c = tex2D(scene, IN.texcoord0.xy).rgb;

	// bloom
	if(params.z > 0.0f)
		c += tex2D(bloom, IN.texcoord0.xy).rgb * params.z;
	...
	if(params.w > 0.0f)
	{
		float d = tex2D(dither, IN.texcoord0.xy).r;
